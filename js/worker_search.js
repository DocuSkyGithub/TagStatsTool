...
self.addEventListener("message", function(e){
   ...
   // 2018-09-04: if corpusnames == ["[ALL]"], needs to get real corpus names from docs
	if (params.corpusnames.length == 1 && params.corpusnames[0] == '[ALL]') {
      // initializing corpuses for '[ALL]'
      for (let i=0; i<doclist.length; i++) {
         let corpus = doclist[i].metadata.Corpus;
         wordclouddata_obj[corpus] = {};
         wordclouddata_arr[corpus] = [];
      }
      console.log(JSON.stringify(wordclouddata_obj));
   }
   else {   // "normal" case
