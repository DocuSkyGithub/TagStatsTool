	...
	*/
	...
	var handler_DocuSkyDocPage = function(){
		...
		for(i = 0, li = docuSkyObj_doc.docList.length; i < li; i++){
         //alert(JSON.stringify(docuSkyObj_doc.docList[i]));
            ...
            var tagkeys = tmp_dsmeta_opt[0].split('/');        // 2017-12-25: (Tu) allows 'timeInfo/dateOrigStr', 'placeInfo/geoLevel1' etc.
            var pass = true;
            ...
            for (var k=0; k<tagkeys.length; k++) {
               ...
               if (node === undefined) {
				...
				//if (elem_docinfo[tagkey] !== undefined){
		...
		mtr.readFiles(files, "text",
			[function(data){
				...
				for(i = 0, li = data.length; i < li; i++){
					...
					tmp_metadata = {
						"filename": fd.name,
						"corpus": "",	// corpus name will be added when user start adding the files to corpus list.
						"order": (i + 1).toString()
					};
